<xsl:stylesheet xmlns="http://www.tei-c.org/ns/1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tei="http://www.tei-c.org/ns/1.0" xpath-default-namespace="http://www.tei-c.org/ns/1.0" exclude-result-prefixes="tei" version="2.0">
      ...
      <xsl:element name="line">
        <xsl:attribute name="xml:id">
         ...
         <xsl:if test="$zone_type">
            <xsl:element name="zone">
                <xsl:attribute name="xml:id">
                    <xsl:value-of select="concat('srcD_zone_', generate-id())"/>
               </xsl:attribute>
               <xsl:attribute name="type">
   ...
   <xsl:template name="lines">
      ...
      <xsl:for-each select="//tei:lb[ancestor::tei:div2/tei:anchor[1]/@xml:id = $anchor_id and @xml:id]">
         <xsl:choose>
            <xsl:when test="not(ancestor::tei:ab) and not(ancestor::tei:note) and not(parent::tei:add) and                   not(following-sibling::tei:ab[1]/preceding-sibling::tei:lb[1]/@xml:id = @xml:id and count(following-sibling::tei:*[local-name() != 'lb' and preceding-sibling::tei:lb[1][@xml:id = current()/@xml:id]]) eq 1)             and not(following-sibling::tei:note[1]/preceding-sibling::tei:lb[1]/@xml:id = @xml:id and count(following-sibling::tei:*[local-name() != 'lb' and preceding-sibling::tei:lb[1][@xml:id = current()/@xml:id]]) eq 1)                   and                      (not(count(following-sibling::tei:lb[1]/preceding-sibling::tei:*[ preceding-sibling::tei:lb[1][@xml:id = current()/@xml:id]]) eq 1 and                      count(following-sibling::tei:lb[1]/preceding-sibling::tei:add[(empty(@place) or @place = 'inline') and preceding-sibling::tei:lb[1][@xml:id = current()/@xml:id]]) eq 1)                      or count(following-sibling::tei:add[1][(empty(@place) or @place = 'inline') and preceding-sibling::tei:lb[1][@xml:id = current()/@xml:id]]/preceding-sibling::text()[normalize-space()]) ne 0                or not(following-sibling::tei:add[1]/tei:lb[1] or count(following-sibling::tei:add[1][empty(@place) or @place = 'inline']/following-sibling::text()[1][normalize-space()]) eq 0))">
